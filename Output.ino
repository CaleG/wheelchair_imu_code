#define FORWARD_THRESH 80
#define BACKWARD_THRESH -100
#define LEFT_THRESH 150
#define RIGHT_THRESH -150
...
{    
  ...
  /*Serial.print("RLL:");
   		...
   		Serial.println();*/
  if(ToDeg(pitch) > FORWARD_THRESH) {
    if(ToDeg(roll) > LEFT_THRESH) {
      Serial.write(0x80);
    }
    else if(ToDeg(roll) < RIGHT_THRESH) {
      Serial.write(0x02);
    }
    else {
      Serial.write(0x01);
  ...
  else if(ToDeg(pitch) < BACKWARD_THRESH) {
    if(ToDeg(roll) > LEFT_THRESH) {
      Serial.write(0x20);
    }
    else if(ToDeg(roll) < RIGHT_THRESH) {
  ...
  else if(ToDeg(roll) > LEFT_THRESH) {
    Serial.write(0x40);
  }
  else if(ToDeg(roll) < RIGHT_THRESH) {
       //    Serial.print(0x04);
